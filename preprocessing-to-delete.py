...
def create_data(param, n, segs=None):
    if segs is None:
        return [augment.get_new_data(param) for i in range(n)]  # n_segments X n_frames
    else:
        return [segment.recreate_image(segs, augment.get_new_data(
...
def create_bm(mat_name, out_name):
    ...
    mask = segment.get_bg_mask(np.reshape(clean_vert[:, 0, 0], [100, 100]))

    # create segmentation for both trials
    segments = segment.vert_horiz_seg(clean_vert_avg_for_seg, clean_horiz_avg_for_seg, mask)     # 100 X 100 integers. same number indicates a single group of pixels

    segments_vert = segment.divide_data_to_segments(segments, clean_vert, mask)       # n_segments X n_frames X n_trials
    segments_horiz = segment.divide_data_to_segments(segments, clean_horiz, mask)     # n_segments X n_frames X n_trials

    recreated_vert = segment.recreate_image(segments, segments_vert)    # 10000 X n_frames X n_trials
    recreated_horiz = segment.recreate_image(segments, segments_horiz)  # 10000 X n_frames X n_trials

    parameters_v = augment.get_parameters(segments_vert)
    parameters_h = augment.get_parameters(segments_horiz)
...
def create_ret(mat_name, out_name):
    ...
    segments_vert = segment.divide_data_to_segments(segments, clean_vert, mask, keep_background=True)       # n_segments X n_frames X n_trials
    segments_horiz = segment.divide_data_to_segments(segments, clean_horiz, mask, keep_background=True)     # n_segments X n_frames X n_trials
